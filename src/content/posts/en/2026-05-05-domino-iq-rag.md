...
cover: "/covers/domino-iq-rag.webp"
coverStyle: "art-deco"
relatedJava: []
relatedSsjs: []
---
...
**1. The vector database always stays local.** In 14.5.1 GA, RAG ran *only* when the Domino IQ server was in Local mode — the whole stack, LLM inference included, ran on your server. **Starting 14.5.1 FP1, RAG works in both Local and Remote modes**, but HCL pins down that "the vector database is always hosted locally on the Domino IQ server" — even in Remote mode. So the vector index and the ACL/Readers filtering layered on top of it never leave your box, and the data-residency argument for financial / legal / healthcare deployments still holds (Remote mode only offloads model inference to a remote OpenAI-compatible endpoint). For that FP1 change, see the [14.5.1 FP1 roundup](/domino-news/en/posts/domino-1451-fp1).
