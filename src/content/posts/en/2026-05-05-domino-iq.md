...
For developers it boils down to: two new LotusScript classes (`NotesLLMRequest` / `NotesLLMResponse`) send a prompt to a local LLM[^llm] and get a response back — no external OpenAI or Anthropic API to wire up, no need to ETL Notes data out.
...
Domino IQ collapses the inference engine and the Domino server into a single process boundary, which handles all three at once. The cost is you have to be able to run a GGUF[^gguf] model on your own server.
...
- optional: guard model[^guard] wiring (filters inappropriate input/output)
