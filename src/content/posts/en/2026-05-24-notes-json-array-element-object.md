...
- **The 64K limit story is version-sensitive** — both the parse-side 64K cap and the "navigator chokes on CR/LF line endings" problem **were fixed in 10.0.1 FP2** ([SPR# DCONB8VMAV](https://ds-infolib.hcltechsw.com/ldd/fixlist.nsf/%28Progress%29/10.0.1%20FP2?OpenDocument) / [ASHEB95LFR](https://ds-infolib.hcltechsw.com/ldd/fixlist.nsf/%28Progress%29/10.0.1%20FP2?OpenDocument)). On 14.5 the only live trap is "**a single element's value exceeding 64K still produces garbled results**" (a LotusScript String engine limit the JSON classes can't route around).
...
| **10.0.1 FP2 onward** | Whole-document JSON > 64K is fine ([**SPR# DCONB8VMAV**](https://ds-infolib.hcltechsw.com/ldd/fixlist.nsf/%28Progress%29/10.0.1%20FP2?OpenDocument) fixed it); **a single element's value > 64K still produces garbled results** |
...
For 14.5 environments (which is most readers): the old "parse-side 64K cap" was fixed seven years ago, and "doesn't handle CRLF" was patched in the same release ([**SPR# ASHEB95LFR**](https://ds-infolib.hcltechsw.com/ldd/fixlist.nsf/%28Progress%29/10.0.1%20FP2?OpenDocument)). **The only live trap left** is ↓
