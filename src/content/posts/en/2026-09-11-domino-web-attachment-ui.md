...
Apply these steps to a real web form and the result looks roughly like this — the footer dump and bare checkboxes are gone, replaced by your own upload area, attachment list with delete, and a custom save button (this is a live Domino web screen; the download list, `$V2AttachmentOptions`, and `%%Detach` are the techniques covered here, with a little extra CSS and a small amount of front-end JS to further polish the upload and delete interactions):

![The beautified Domino web attachment UI, mid-delete: a dashed-border upload area at top with a custom "Choose file" button and tags for the selected files; in the bordered attachment list, two rows are struck through and greyed with their buttons switched to "Undo" (marked for deletion, but reversible until you save), while two rows show the normal blue download link and a "Delete" button; a custom green "Save" button sits at the bottom right — replacing the crude default from the opening](/domino-news/post-images/domino-web-attachment-beautified.png)
