...
cover: "/covers/notes-dom-parser.png"
coverStyle: "art-deco"
---
...
1. **整棵載入記憶體** — 跟昨天 SAX 那篇相反、適合**小到中型 XML + 需要任意走訪或修改**
...
跟昨天 [NotesSAXParser 文](/domino-news/posts/notes-sax-parser) 對比：SAX 是 forward-only streaming、DOM 是 random-access tree。今天看 DOM。
...
明天的對比文整理「**何時 SAX、何時 DOM、何時 NotesXMLProcessor（XSLT）**」的決策樹 — 三條 XML 路線一次釐清。
