...
description: "把 Domino 包進 container 在 IBM 時代（V9）就有社群在做、HCL 從 V10 起正式提供官方 pre-built container image 下載。現在 HCL 雙管齊下：要快上線可以從 My HCLSoftware Portal 下載官方 pre-built image TAR 直接 docker load；要彈性可以用 HCL 在 GitHub 上開源的 domino-container 專案、跑互動式 build.sh menu 自己 build 客製 image，挑你要的模組（Domino / Traveler / Verse / Nomad / REST-API / Leap / Domino IQ / OnTime / C-API SDK / LP）+ add-on。這篇介紹兩條路怎麼選、build.sh 能客製到什麼程度、典型部署場景，跟想試試看的人怎麼起步。"
...
- HCL 提供**兩條路**：
  - **(Path A)** 從 [My HCLSoftware Portal](https://my.hcltechsw.com/downloads/domino/domino) 下載官方 pre-built container image TAR、`docker load` 進來 — 快、適合標準部署
...
**(A) Pull pre-built image** — HCL 在 [My HCLSoftware Portal 的 Domino downloads](https://my.hcltechsw.com/downloads/domino/domino) 提供 ready-to-go 的 container image 給有 entitlement 的客戶下載 TAR 檔、回到本機 `docker load` 進來就能跑。這是最快上線的方式。
...
```bash
# 到 My HCLSoftware Portal (https://my.hcltechsw.com/downloads/domino/domino)
# 用 entitlement 帳號下載 Domino container image TAR 檔
docker load -i Domino_14.5.1_container.tar
# load 完 docker images 看一下 image 名 + tag、然後跑起來
...
Container 化是 Domino 現代化部署的一條主要路線、HCL 雙管齊下：要快上線的從 [My HCLSoftware Portal](https://my.hcltechsw.com/downloads/domino/domino) 下載 pre-built image TAR 直接 `docker load`、要客製的用 [open-source build script](https://github.com/HCL-TECH-SOFTWARE/domino-container) 自己 build。
