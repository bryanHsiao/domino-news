...
assono 那篇已經十多年，中間 Domino 換了好幾個大版本。所以我們照他的樣本做了一支最小 XPage（`save → 改後端 → 再 save`，中間留 3 秒時間差），在 **Domino 12.0.2** 與 **14.5.1** 上各跑了幾次，看這個坑現在還在不在。

結論很直接：**兩版都還會，而且每跑一次就生出一份衝突文件——穩定發作、不是偶發。** 我們用一個「掃全庫 `$Conflict` 數」的按鈕比對：每按一次「跑重現」，帶 `$Conflict` 的文件數就 **+1**（12.0.2 與 14.5.1 皆然）。

更直接的是，頁面上的 `xp:messages` 當場就把那句經典訊息吐了出來：

> 「其他使用者已儲存文件 -『儲存』已建立一份新文件作為該已修改文件的回應。」

——從頭到尾只有我們這一支程式在動這份文件，卻跳出「**其他使用者**已儲存」。這正是開頭那個弔詭的真身：Domino 的衝突判定只看「有沒有兩條血脈」，不看「幾個人」。

![存檔衝突重現的實測畫面：頁面上的 xp:messages 跳出黃色訊息「其他使用者已儲存文件 -『儲存』已建立一份新文件作為該已修改文件的回應。」，Note 欄位顯示後端改入的值——但從頭到尾只有這支測試程式在動這份文件（Domino 12.0.2／14.5.1 實測）](/domino-news/post-images/xpages-save-conflict-message.png)

換句話說，這個 2013 年的坑**到 14.5.1 依然完全成立**——它不是某個舊版的暫時性 bug，而是「在同一份文件上混用兩套 save」的必然結果。

**而時間差就是那個扳機。** 留著 3 秒 `Thread.sleep` 時，兩版每次必中；把兩個 `Thread.sleep` 拿掉、讓兩次存檔擠在同一瞬間，就**不會生衝突**了。這跟 assono 的觀察一致，也正好解釋了為什麼這個坑實務上「偶爾才中」：會不會撞，取決於兩次存檔之間有沒有拉開足夠的時間差（`$Revisions` 是用時間戳判定的）。伺服器慢一點、或兩次存檔之間夾了個耗時操作，那道縫一開，衝突就冒出來——這也是它為什麼最難查。
