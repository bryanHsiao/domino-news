...
| 層 | 是什麼 | 14.5 預設 |
|---|---|---|
| **Layer 1：per-port 加密能力** | server document → Ports → Notes Network Ports → 每個 port 的「[Encrypt network data](https://help.hcl-software.com/domino/14.5.0/admin/conf_encryptingnrpccommunicationonaserverport_t.html)」checkbox | **打開**（fresh install，HCL 14.5 文件原文：「[NRPC port encryption is enabled for increased security level](https://help.hcl-software.com/domino/14.5.0/admin/conf_port_encryption_t.html)」） |
| **Layer 2：強制 mandate** | Directory Profile 的 Mandated Port Encryption 設定（本文主題） | **停用**（你看到的 `?` icon 來源） |

### Layer 1 在做什麼

NRPC（Notes Remote Procedure Call）是 server 之間、Notes client 連 server 都在跑的通訊協議。Layer 1 的「Encrypt network data」checkbox 在 Domino 早就存在很多版本，[HCL 官方文件](https://help.hcl-software.com/domino/14.5.0/admin/conf_encryptingnrpccommunicationonaserverport_t.html)說它的威脅模型是 "prevent the network eavesdropping that's possible with a network protocol analyzer" —— 防止有人在你網路上跑 wireshark / tcpdump 抓 NRPC 封包。

但 Layer 1 有個關鍵細節：[同一份文件](https://help.hcl-software.com/domino/14.5.0/admin/conf_encryptingnrpccommunicationonaserverport_t.html)寫 "Network data encryption occurs if you enable network data encryption on either side of a network connection" —— 任一邊啟用就加密、典型情境是「server 開了、client 自然吃到」。聽起來好，但反過來：**只要對方因為任何理由（版本太舊、設定上拒絕、配置漂移）不接受加密，那條連線可能就跑明文**。Admin 在 Layer 1 沒辦法強制「全 domain 每條 link 都加密」這件事。

### Layer 2 補的洞

Layer 2 就是補這個強制權。[HCL Mandated Port Encryption 官方頁](https://help.hcl-software.com/domino/14.5.0/admin/mandated_port_encryption.html)的原文：「Enables and enforces NRPC port encryption on both the client and server. If configured by an administrator, encryption needs to be enforced even if the other side does not want to use encryption」—— 「即使對方不想，也要強制」。
...
[HCL Layer 1 文件](https://help.hcl-software.com/domino/14.5.0/admin/conf_encryptingnrpccommunicationonaserverport_t.html)寫的威脅模型是「network eavesdropping ... with a network protocol analyzer」—— 假設攻擊者已經能在你網路上跑封包擷取。**內網辦公**視為這個風險低、**跨網段 / 公網**視為高。「lateral movement」「合規稽核」這些是業界共通的延伸考量、HCL 文件本身沒列、但實務上影響啟用 Mandated 的決策。
