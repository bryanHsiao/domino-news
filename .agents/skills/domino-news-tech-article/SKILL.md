...
scheduling pattern, recent fixes) live in `AGENTS.md` at the repo
...
- **News articles** (release notes, OpenNTF tool intros, ecosystem
  ...
  standard editorial conventions in AGENTS.md but skip the steps
  here.
...
The detailed editorial conventions live in `AGENTS.md` under
...
Java- or SSJS-only post (per the C plan in AGENTS.md). Internal
...
see AGENTS.md "Scheduling pattern" for the design rationale.
...
and any cross-references to companion posts. AGENTS.md commit-style
...
(see AGENTS.md "One-time setup for cron attribution").
...
- `AGENTS.md` — project memory: editorial voice, frontmatter schema,
