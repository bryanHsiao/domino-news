...
description: "「把這個 view 匯出 Excel 給 user」是 Domino 圈最常 ask 的需求之一。四條路 — CSV via NotesStream（推薦預設）、HTML-as-xls（legacy 格式技巧）、OLE Excel.Application（client-only ad-hoc）、Apache POI via Java agent（production-grade .xlsx） — 各有適用場景。本文整理四條路的 trade-off 表、CSV 完整實作（含 UTF-8 BOM、CSV escape、用 NotesViewNavigator 加速 5-10x）、HTML-as-xls / OLE / POI 的何時用何時不該用、Web download pattern、跟三個常見坑（BOM 漏寫、escape 不對、用 GetFirstDocument 而不是 ViewNavigator）。"
...
- 要 .xlsx 真檔 + formatting → Apache POI Java agent；client-only ad-hoc → OLE；要 cell 顏色但不想搞 POI → HTML-as-xls
...
| 路 | 機制 | 跑在哪 | 適合 | 不適合 |
|---|---|---|---|---|
| **1. CSV（推薦預設）** | NotesStream 寫 .csv、Excel 直接開 | 任何地方（LS only）| 純資料 dump、排程報表、大量資料 | 要 formatting / 多 sheet / 公式 |
| **2. HTML-as-xls** | 寫 `<table>` HTML、副檔名取 `.xls` | 任何 LS | 要簡單 cell 顏色 / 框線、一次性報表 | Excel 開會跳「這不是 .xls」警告、Microsoft 已 deprecated |
| **3. OLE Excel.Application** | `CreateObject("Excel.Application")` 操控 Excel | **僅 Windows + Excel 已裝** | desktop ad-hoc script、要 pivot / 圖表 | server agent 不能用（Linux Domino、沒裝 Excel） |
| **4. Apache POI via Java agent** | Java agent 用 POI 寫 .xlsx、LS 用 LS2J 呼叫 | server-side、production | 真 .xlsx、formatting、大量資料、Linux Domino | 多一個 dependency、Java agent 學習曲線 |

實務上 9 成需求 CSV 就解了 — 只有「要 formatting」/「要 .xlsx 真檔」才升 POI。
...
- 不是真 .xlsx — 存檔時 Excel 會問要不要轉成 .xlsx 才能保 formatting
- Microsoft 從 Office 2007 開始把這條路 deprecated、長期不建議

適用場景：**內部用、一次性、需要簡單 formatting** 的報表。Production 別走這條。

---

## 路 3：OLE Excel.Application — Windows desktop only
...
- ✅ desktop 跑的 client agent、user 機器上有 Excel、要做 pivot / chart / 複雜公式
- ❌ Server agent（Linux Domino 沒 Excel、Windows server 通常也不裝）
- ❌ 高頻 / 高量 — OLE call overhead 大、跑 10K rows 可能要 30 秒以上
- ❌ Excel process leak 風險 — 沒 `xlApp.Quit()` 或 exception 跳出沒 cleanup、process 留在 Task Manager

---

## 路 4：Apache POI via Java agent — production-grade .xlsx

要真 .xlsx + production-grade 穩定性：**Java agent + [Apache POI](https://poi.apache.org/)**。
...
- 真 .xlsx — Excel 開不會跳警告、保留 formatting / formulas / multiple sheets
...
實務上 production-grade 報表的標準做法就是這條 — 痛一次設好之後可以重用。
...
| 要簡單 cell 顏色、internal 用、一次性 | HTML-as-xls |
| Desktop ad-hoc、Excel 已裝、要 pivot/chart | OLE Excel.Application |
| 真 .xlsx、production-grade、formatting / 多 sheet | Apache POI Java agent |
...
| Java | Apache POI（XSSFWorkbook / SXSSFWorkbook）— 業界標準、production .xlsx 都走這條 |
