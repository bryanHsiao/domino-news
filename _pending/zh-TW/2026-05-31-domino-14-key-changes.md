...
- **JAR 放的位置 14 起不能再用 `jvm/lib/ext`** — V12（Java 8）時放這裡會自動掃進 classpath、實戰確認可以；14.0 主 JVM 升 [Java 17 後 ext mechanism 完全移除](https://openjdk.org/jeps/220)（Java 9 deprecated、Java 11 砍）、Domino 不會再從那裡掃 jar、實務 fallback 是 `ndext`
...
### JAR 放置位置 — Java 17 移除 ext mechanism 的連帶影響

V11 / V12（Java 8 時代）保留 [ext class loader 機制](https://openjdk.org/jeps/220)、jar 放 `<domino>/jvm/lib/ext/` 會被 JVM 自動掃進 classpath。實戰上 12 版這條路是 work 的、許多自家 extension 都靠這個位置 deployment。

**從 14.0 起、主 JVM 升 Java 17** — Java 9 deprecated 了 ext mechanism、Java 11 完全砍掉。14.x 跑 Java 17、`jvm/lib/ext` 已經完全不被掃進 classpath、原本放那邊的 jar 直接失效。這是 V12 → 14 升級時非常容易踩雷的點 — class load fail 的錯誤訊息常常讓人花很多時間在錯的方向 debug。

要搬到哪？實務上兩條 path 可以分開講：

- **`JSDK.JAR`（servlet 開發用的 Java Servlet Development Kit）**：[官方明文](https://help.hcl-software.com/dom_designer/14.5.0/basic/H_RUNNING_SERVLETS_IN_DOMINO.html)指 `<domino>/ndext/`。這條不是 14.5 才加的、[Domino 12.0 同一頁](https://help.hcl-software.com/dom_designer/12.0.0/basic/H_RUNNING_SERVLETS_IN_DOMINO.html)就有同樣文字 — 寫 servlet 編譯時要找新路徑找 classpath
- **自家 / 第三方 extension jar**：HCL 沒對「all extension jars 統一搬 `ndext`」做正式 announcement、但實務 fallback 是 `ndext`（跟 `JSDK.JAR` 同一個目錄）。其他更乾淨的選擇是包 OSGi plugin 或在 server 設定明確聲明 classpath

```
V12 時可以：  <domino>/jvm/lib/ext/yourlib.jar   （Java 8 ext mechanism 掃）
14.x 改成：  <domino>/ndext/yourlib.jar          （或 OSGi / 明確 classpath）
```

升級時：把所有原本 deploy 在 `jvm/lib/ext` 的自家 / 第三方 jar 列出來、逐一搬到 `ndext` 或改 load 機制。不搬的話、相關 class load 一定 fail。
...
- [ ] 把 V12 時放 `jvm/lib/ext` 的自家 / 第三方 jar 都搬到 `ndext`（或改 OSGi / 明確 classpath） — Java 17 移除 ext mechanism、14.x 之後這個位置完全不被掃。`JSDK.JAR` 早就在 `ndext`（12.0 就有）、但一般 extension jar 在 V12 還能放 `jvm/lib/ext`、14 之後必須搬
